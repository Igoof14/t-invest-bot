"""Тесты получения идентификаторов облигаций портфеля (общее)."""
...
import pytest
from features.ratings import portfolio
...
def _patch_client(
    monkeypatch: pytest.MonkeyPatch, *, accounts: list[MagicMock], get_portfolio: AsyncMock
) -> None:
    client = MagicMock()
    ...
    monkeypatch.setattr(portfolio, "AsyncClient", MagicMock(return_value=cm))
...
async def test_collects_figi_and_ticker_of_bonds(monkeypatch: pytest.MonkeyPatch) -> None:
    pf = MagicMock(
        positions=[
            ...
            _position("share", figi="BBG00AAPL", ticker="AAPL"),
            _position("bond", figi="", ticker="RU000A0JX0J2"),
        ]
    )
    _patch_client(monkeypatch, accounts=[_account("a1")], get_portfolio=AsyncMock(return_value=pf))

    result = await get_portfolio_bond_identifiers("token")
    assert result == {"BBG00X", "RU000A105RV3", "RU000A0JX0J2"}


async def test_returns_empty_on_error(monkeypatch: pytest.MonkeyPatch) -> None:
    cm = MagicMock()
    cm.__aenter__ = AsyncMock(side_effect=RuntimeError("auth failed"))
    cm.__aexit__ = AsyncMock(return_value=False)
    monkeypatch.setattr(portfolio, "AsyncClient", MagicMock(return_value=cm))

    assert await get_portfolio_bond_identifiers("token") == set()
