"""Тесты парсинга и нормализации пула прокси НРД."""

from __future__ import annotations

import pytest
from features.nsd_coupons import proxy_pool
from features.nsd_coupons.proxy_pool import load_proxies, normalize_proxy
...
def test_load_proxies_empty_returns_direct(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(proxy_pool.config, "nsd_proxies", None)
    monkeypatch.setattr(proxy_pool.config, "nsd_proxy", None)
    assert load_proxies() == [None]


def test_load_proxies_falls_back_to_singular(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(proxy_pool.config, "nsd_proxies", None)
    monkeypatch.setattr(proxy_pool.config, "nsd_proxy", "1.2.3.4:8080")
...
def test_load_proxies_skips_bad_entries(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(proxy_pool.config, "nsd_proxies", "garbage, 5.6.7.8:9090")
    monkeypatch.setattr(proxy_pool.config, "nsd_proxy", None)
