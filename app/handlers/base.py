...
def register_handlers(dp: Dispatcher, bot: Bot):
    ...
    async def handle_buttons(message: Message):
        ...
        if message.text == "Купоны сегодня":
            ...
            await message.answer(
                get_coupon_payment(start_datetime=today_midnight), parse_mode="HTML"
            )
            ...
            await message.answer(
                get_coupon_payment(start_datetime=start_datetime), parse_mode="HTML"
            )
...
async def set_commands(bot: Bot):
    ...
    commands = [
        ...
        BotCommand(command="inline_url", description="Инлайн ссылка"),
        BotCommand(command="random", description="Случайное число"),
    ]
    await bot.set_my_commands(commands)


async def send_report(bot: Bot, report_type: ReportType):
    """Рассылка отчёта."""
    if not user_ids:
        return
