"""Пул прокси для запросов к НРД: парсинг и нормализация в URL."""
...
def load_proxies() -> list[str | None]:
    """Загружает пул прокси для НРД из конфигурации.

    Берёт ``config.nsd_proxies`` (список), иначе одиночный ``config.nsd_proxy``.
    Если ничего не задано — возвращает ``[None]`` (прямое соединение).

    Returns:
        Список URL прокси; ``[None]`` означает одно «прямое» соединение.

    """
    raw = config.nsd_proxies or config.nsd_proxy
    if not raw:
        return [None]
